<template>
  <div class="data-box">
    <div>
      <head-line :title="'质量销项统计分析'"></head-line>
      <el-date-picker style="margin-top:16px"
        class="date-input"
        format="yyyy-MM-dd"
        v-model="sales_time"
        type="daterange"
    ...
    <div>
      <head-line :title="'质量问题项统计'"></head-line>
      <div>
        <el-date-picker
        class="date-input"
        format="yyyy-MM-dd"
      ...
      <echarts-circle class="echarts-circle" :data="problem_statistics_data"></echarts-circle>
      <small-table :data="problem_statistics_data" class="echarts-circle table"></small-table>
    </div>
    <div>
      <head-line :title="'质量整改单统计(' + year + ')年'"></head-line>
      <echarts-line class="echarts-line" :data="annual_survey_data" v-if="annual_survey_data"></echarts-line>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      websocketRoom: 'quality_data_center',
      websocketType: 'quality',
   ...
   methods: {
    ...
    check() {
      ...
      if(this.is_fetch_auth) {
        ...
        this.sales_statistics_alalysis_data = undefined
        this.penging_sales_problem_statistics_data = undefined
        this.problem_statistics_data = undefined
        this.annual_survey_data = undefined
      }
    }
  },
  // mounted() {
